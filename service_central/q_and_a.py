...
mongo_setup.dealership_init()

print('Time to ask a question')

timed(
    'How many owners?',
    lambda: Owner.objects().count()
)
...
timed(
    'Find the 10,000th owner by car?',
...
timed(
    'How many owners ownt the 10,000th car?',
    lambda: owners_by_the_car(car.id)
)
